...
function fieldItem(name: string): DesignerItem {
  return { id: 'f1', field: { name, type: 'TEXT', label: '姓名', colSpan: 24, subFields: [] } }
}
...
describe('FieldConfigPanel 字段标识（V011-BUG-016）', () => {
  it('未发布：只展示 field_ 后缀，输入后缀回写全名并规范化', async () => {
    ...
    const keyInput = wrapper.findAll('.config__row input').find((i) => (i.element as HTMLInputElement).value === 'username')!

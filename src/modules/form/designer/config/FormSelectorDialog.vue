...
const props = defineProps<{
  visible: boolean
  /** 当前已选的 targetFormId（formKey），用于回显高亮。 */
...
<template>
  <el-dialog
    :model-value="visible"
    :title="t('form.targetFormDialogTitle')"
  ...
  >
    <div class="form-selector">
      ...
      <el-alert type="info" :closable="false" show-icon style="margin-bottom: var(--sw-space-16)">
        <template #title> {{ t('form.publishedOnlyTargetHint') }} </template>
      </el-alert>

      <!-- 搜索栏 -->
      <div class="form-selector__search">
        <el-input
          v-model="keyword"
      ...
      <!-- 分页 -->
      <div class="form-selector__pagination">
        <span class="form-selector__pagination-total">{{
...
.form-selector__search {
  display: flex;
  align-items: center;
  gap: var(--sw-space-8);
}

.form-selector__pagination {
  display: flex;
  align-items: center;
...
.form-selector__pagination-total {

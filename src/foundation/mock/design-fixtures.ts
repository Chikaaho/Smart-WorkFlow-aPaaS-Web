...
/** 设计壳层（顶栏/侧栏）fixture 菜单：复用原叶子节点注册路由，仅重组分组与标题。 */
  ...
  return [
    ...
    { ...leaf('agent/conversations/list', '智能助手'), hidden: true, topbar: 6, topbarArea: 'portal' },
...
const OVERRIDES: Record<string, Record<string, DesignMockHandler>> = {
  ...
  node01: {
      ...
      ok({
        ...
        layout: {
          components: [
            ...
            { key: 'myProcessed', visible: true, order: 3 },
            { key: 'myInitiated', visible: true, order: 4 },
            { key: 'cc', visible: true, order: 5 },
            { key: 'drafts', visible: false, order: 6 },
            { key: 'messages', visible: false, order: 7 },
          ],
          favoriteItemKeys: [
            'TRAVEL_EXPENSE',
            'ASSET_PURCHASE',
    ...
    'GET /api/workflow/drafts': () =>
      ok({
        records: [
          { id: 1, formKey: 'travel-expense', title: '差旅报销申请', status: 'DRAFT', updateTime: '2026-09-15 10:12:00' },
          { id: 2, formKey: 'asset-purchase', title: '资产采购申请', status: 'DRAFT', updateTime: '2026-09-14 16:40:00' },
          { id: 3, formKey: 'kb-permission', title: '知识库权限申请', status: 'DRAFT', updateTime: '2026-09-13 09:05:00' },
        ],
        total: 3,
        pageNum: 1,
        pageSize: 5,
  ...
  node15: taskDetailWithHistory([
    historyRow({
      ...
      opinionData: { comment: '情况属实，同意按差旅标准执行。', 紧急程度: '普通', 值守负责人: '陈建国', 已确认检查项: ['主备状态检查', '回退预案确认', '值守安排确认'], 附件: '2 份' },
    }),
  ]),
  ...
  node17: taskDetailWithHistory([
    historyRow({
      ...
      opinionData: { comment: '设备确已到货验收，同意入账。', 验收单号: 'YS-2026-0914-07', 值守负责人: '周敏', 已确认检查项: ['到货验收', '入账确认', '值守安排确认'], 附件: '验收单_Y S.pdf' },
    }),
  ]),
  node18: taskDetailWithHistory([
    historyRow({
      ...
      opinionData: { comment: '按合同条款执行，请财务复核付款节点。', 合同编号: 'HT-2026-0331', 值守负责人: '李宁', 已确认检查项: ['主备状态检查', '回退预案确认', '值守安排确认'], 附件: '会签检查单.pdf' },
    }),
  ]),
      ...
      ok({
        ...
        approvalHistory: [
          ...
          },
          {
            ...
            opinionData: { comment: '同意按计划开展灾备演练。请科技负责人会签确认技术方案，并落实演练期间的值守与回退安排。', 审批结论: '同意', 风险评估: '风险可控', 是否补充演练: '否', 值守负责人: '周明', 已确认检查项: ['主备状态检查', '回退预案确认', '值守安排确认'], 附件: '部门审核清单.pdf' },
          },
          {
            taskId: 'h-2',
            nodeKey: 'joint-sign',
          ...
          },
          {
            ...
            opinionData: { comment: '验证方案满足主备切换要求；建议演练前完成设备检查及回退验证，并确认所有值守人员到位。', 审批结论: '同意', 风险评估: '风险可控', 是否补充演练: '否', 值守负责人: '陈曦', 已确认检查项: ['主备状态检查', '回退预案确认', '值守安排确认'], 附件: '灾备会签检查单.pdf' },
          },
          {
            taskId: 'h-4',
            nodeKey: 'joint-sign',
            ...
            opinionFormId: '灾备会签意见表单',
            opinionFormVersion: '1.2',
            opinionData: { comment: '基础设施检查已完成，主设备与机房电力状态正常。确认回退预案与值守安排，同意执行本次演练。', 审批结论: '同意', 风险评估: '风险可控', 是否补充演练: '否', 值守负责人: '李宁', 已确认检查项: ['主备状态检查', '回退预案确认', '值守安排确认'], 附件: '基础设施会签检查单.pdf' },
          },
          {
            taskId: 'h-5',
            nodeKey: 'joint-sign',
  ...
  node05: {
    ...
    'GET /api/workflow/tasks/todo': () =>
      ok({ records: [], total: 6, pageNum: 1, pageSize: 1 }),
    'GET /api/workflow/monitor/analytics/summary': () =>
      ok({
        launched: 706,
        completed: 695,
...
function designProcessGraphPayload(): Record<string, unknown> {
  return {
    ...
    elements: [
      ...
      { id: 'draft', kind: 'node', type: 'APPROVAL', x: 416, y: 145, config: { name: '起草节点', nodeClass: 'draft', approver: JSON.stringify({ type: 'DESIGNATED', value: ['2', '103'] }) } },
      { id: 'dept', kind: 'node', type: 'APPROVAL', x: 416, y: 236, config: { name: '部门负责人审批', approver: JSON.stringify({ type: 'DESIGNATED', value: ['2', '103'] }) } },
      { id: 'tech', kind: 'node', type: 'APPROVAL', x: 416, y: 327, config: {
        name: '办公室资产管理员意见',
        ...
        listeners: [
          { phase: 'before', label: '节点进入前', bean: 'assetApprovalListener', note: 'Bean 调用 · 已启用' },
          { phase: 'after', label: '审批完成后', note: '同步采购状态 / 发送业务事件' },
        ],
      } },
      { id: 'gateway', kind: 'node', type: 'GATEWAY', x: 416, y: 485, config: { name: '分支 / 汇聚' } },
      { id: 'risk', kind: 'node', type: 'APPROVAL', x: 211, y: 483, config: { name: '资产核价' } },
      { id: 'compliance', kind: 'node', type: 'APPROVAL', x: 211, y: 591, config: { name: '合规部负责人意见' } },
      { id: 'plan', kind: 'node', type: 'APPROVAL', x: 691, y: 254, config: { name: '资产采购' } },
      { id: 'branch', kind: 'node', type: 'APPROVAL', x: 828, y: 343, config: { name: '分公司领导审核' } },
      { id: 'divide', kind: 'node', type: 'APPROVAL', x: 828, y: 451, config: { name: '分管领导审批' } },
      { id: 'finance', kind: 'node', type: 'APPROVAL', x: 828, y: 559, config: { name: '财务部审核' } },
      { id: 'sign', kind: 'node', type: 'APPROVAL', x: 828, y: 667, config: { name: '会签部门意见' } },
      ...
      { id: 'e15', kind: 'edge', source: 'tech', target: 'compliance', config: { sourceAnchor: [336, 327], targetAnchor: [131, 591] }, waypoints: [{ x: 79.5, y: 327 }, { x: 79.5, y: 591 }] },
      // 设计稿网关扇出：入 plan 底边中点与右列节点左下角（锁定 SVG 端点实测）
      { id: 'e9', kind: 'edge', source: 'gateway', target: 'plan', config: { sourceAnchor: [438, 485], targetAnchor: [691, 279] } },
      { id: 'e10', kind: 'edge', source: 'gateway', target: 'branch', config: { sourceAnchor: [438, 485], targetAnchor: [748, 368] } },
      { id: 'e11', kind: 'edge', source: 'gateway', target: 'divide', config: { sourceAnchor: [438, 485], targetAnchor: [748, 476] } },
      { id: 'e12', kind: 'edge', source: 'gateway', target: 'finance', config: { sourceAnchor: [438, 485], targetAnchor: [748, 584] } },
      { id: 'e13', kind: 'edge', source: 'gateway', target: 'sign', config: { sourceAnchor: [438, 485], targetAnchor: [748, 692] } },
  ...
  return {
    ...
    'GET /api/workflow/defs/node-capabilities': () =>
      ok([
        ...
        { type: 'DRAFT', displayName: '起草', description: '设计参考项', category: 'TASK', version: 'design-only', topology: { minIncoming: 0, maxIncoming: 1, minOutgoing: 1, maxOutgoing: 1 }, configFields: [], supports: { design: false, save: false, publish: false, run: false } },
        MOCK_WORKFLOW_NODE_CAPABILITIES.find((cap) => cap.type === 'APPROVAL'),
        MOCK_WORKFLOW_NODE_CAPABILITIES.find((cap) => cap.type === 'CONSENSUS'),
        { ...MOCK_WORKFLOW_NODE_CAPABILITIES.find((cap) => cap.type === 'CONDITION'), displayName: '条件网关' },
        MOCK_WORKFLOW_NODE_CAPABILITIES.find((cap) => cap.type === 'NOTIFICATION'),
        { type: 'IOT_COMMAND', displayName: 'IoT 指令', description: '设计参考项', category: 'TASK', version: 'design-only', topology: { minIncoming: 1, maxIncoming: 1, minOutgoing: 1, maxOutgoing: 1 }, configFields: [], supports: { design: false, save: false, publish: false, run: false } },
        { type: 'AGENT', displayName: 'Agent', description: '设计参考项', category: 'TASK', version: 'design-only', topology: { minIncoming: 1, maxIncoming: 1, minOutgoing: 1, maxOutgoing: 1 }, configFields: [], supports: { design: false, save: false, publish: false, run: false } },
...
function taskGraphPayload(): Record<string, unknown> {
  return {
    ...
    elements: [
      ...
      { id: 'dept', kind: 'node', type: 'APPROVAL', x: 419, y: 230, config: { name: '部门负责人审批' } },
      { id: 'tech', kind: 'node', type: 'APPROVAL', x: 419, y: 321, config: { name: '科技运营负责人意见' } },
      { id: 'gateway', kind: 'node', type: 'GATEWAY', x: 419, y: 479, config: { name: '分支汇聚' } },
      { id: 'risk', kind: 'node', type: 'APPROVAL', x: 214, y: 477, config: { name: '风险核查' } },
      { id: 'compliance', kind: 'node', type: 'APPROVAL', x: 214, y: 585, config: { name: '合规负责人意见' } },
      { id: 'plan', kind: 'node', type: 'APPROVAL', x: 694, y: 248, config: { name: '灾备方案审核' } },
      { id: 'branch', kind: 'node', type: 'APPROVAL', x: 831, y: 337, config: { name: '分公司领导审核' } },
      { id: 'divide', kind: 'node', type: 'APPROVAL', x: 831, y: 445, config: { name: '分管领导审批' } },
      { id: 'finance', kind: 'node', type: 'APPROVAL', x: 831, y: 553, config: { name: '财务部审核' } },
      { id: 'sign', kind: 'node', type: 'APPROVAL', x: 831, y: 661, config: { name: '科技负责人会签' } },
      { id: 'end', kind: 'node', type: 'END', x: 419, y: 742, config: { name: '结束节点' } },
      // 真实流程尾部（设计20 右栏『后一节点』）：仅逻辑后继；边不渲染（设计19 无会签出线）。
      { id: 'gm', kind: 'node', type: 'APPROVAL', x: 830, y: 990, config: { name: '总公司科技部审批' } },
      { id: 'e1', kind: 'edge', source: 'start', target: 'draft' },
      { id: 'e2', kind: 'edge', source: 'draft', target: 'dept' },
      { id: 'e3', kind: 'edge', source: 'dept', target: 'tech' },
      { id: 'e4', kind: 'edge', source: 'tech', target: 'gateway' },
      ...
      { id: 'e20', kind: 'edge', source: 'compliance', target: 'tech', waypoints: [{ x: 83, y: 585 }, { x: 83, y: 321 }] },
      { id: 'e9', kind: 'edge', source: 'gateway', target: 'plan', waypoints: [{ x: 694, y: 273 }] },
      { id: 'e10', kind: 'edge', source: 'gateway', target: 'branch', waypoints: [{ x: 751, y: 362 }] },
      { id: 'e11', kind: 'edge', source: 'gateway', target: 'divide', waypoints: [{ x: 751, y: 470 }] },
      { id: 'e12', kind: 'edge', source: 'gateway', target: 'finance', waypoints: [{ x: 751, y: 578 }] },
      { id: 'e13', kind: 'edge', source: 'gateway', target: 'sign', waypoints: [{ x: 751, y: 686 }] },
      { id: 'e14', kind: 'edge', source: 'gateway', target: 'end' },

...
**本文件所在目录（`Smart-WorkFlow-Web/`）的会话角色是执行（前端）。**

本目录会话角色为**执行（前端）**，由用户在会话开始时声明（工作区根 `../../../system.md` §0.2）；planning layer 位于工作区根目录 `/usr/local/projects/Smart-WorkFlow`。

你的职责边界：

- ✅ 接收规划层下发的**需求方向文档**（目标/非目标/影响范围/风险），**自行拆分 Step、设计每个 Step 的执行/测试方案**并自主执行（根目录 `roles/executor.md` §4 自主闭环）
- ✅ 运行校验门（四连：`typecheck && lint && test && build`），产出结构化执行回执和测试回执（按根目录 `roles/executor.md` §8 格式）
...
>
> **终态命名**：执行任务结束只使用 `EXECUTION_SUBMITTED` / `TERMINAL_SYNC_SUBMITTED` / `BLOCKED`；不得以 `执行终态：COMPLETED` 混淆功能状态。阶段三边界以根 `system.md` §3.4 与 `roles/executor.md` §4.2 为准。
>
...
0. **角色与授权优先**：先遵守根 `system.md` 的角色门禁。已声明执行角色且给出需求方向/审查记录/任务路径时，视为一次性完整授权，直接按 `roles/executor.md` §4 闭环，不再要求二次确认；未声明角色则拒绝操作；方向缺失或内部冲突才报告 `BLOCKED`。
...
> 本条款为根 system.md §0.4「信息分层铁律（D85，2026-08-16）」在前端工程宪法的同步条目，语义与根表述一致，冲突时以根 system.md 为准。
